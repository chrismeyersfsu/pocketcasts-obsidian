...
interface TemplaterPlugin {
	templater: {
		write_template_to_file(templateFile: TFile, noteFile: TFile): Promise<void>;
	};
}
...
export default class PocketCastsPlugin extends Plugin {
	...
	async onload() {
		...
		this.addCommand({
			...
			callback: async () => {
				...
				if (leaf) {
					void (leaf.view as PocketCastsView).refresh();
				} else {
	...
	onunload() {
		// intentionally empty — do not detach leaves so user's view placement is preserved
	}
